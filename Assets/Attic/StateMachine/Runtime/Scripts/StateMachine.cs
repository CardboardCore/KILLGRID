...
namespace Attic.StateMachines
{
    ...
    public abstract class StateMachine
    {
        ...
        protected readonly Dictionary<State, Transition> globalTransitionDict = new Dictionary<State, Transition>();
        ...
        // Method to add a global transition to a specific state
        public TTo AddGlobalTransition<TTo>()
            where TTo : State, new()
        {
            State to = CreateState<TTo>();

            if (!globalTransitionDict.ContainsKey(to))
            {
                // Use null as from-state to indicate "any"
                Transition transition = new Transition(null, to);
                globalTransitionDict[to] = transition;
            }
        ...
        public void Start()
        {
            foreach (KeyValuePair<Type, State> item in stateDict)
            {
        ...
        // Method to perform a global transition to a specific state
        public void ToGlobalState<TTo>()
            where TTo : State, new()
        {
            State to = GetState<TTo>(true);

            if (globalTransitionDict.TryGetValue(to, out Transition transition))
            {
                transition.Do(out currentState, this);
            }
            else
            {
                Debug.LogWarning($"No global transition found to state {typeof(TTo).Name}");
            }
        }
    }
}
